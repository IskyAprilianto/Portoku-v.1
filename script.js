document.addEventListener('DOMContentLoaded', () => {
    ...
    if (resTrack && resDotsContainer) {
        ...
        // Autoplay for Research Slider (5 seconds)
        let resAutoplayTimer = null;
        const RES_AUTOPLAY_DELAY = 5000;
        
        const startResAutoplay = () => {
            stopResAutoplay();
            resAutoplayTimer = setInterval(() => {
                let nextIndex = (resCurrentIndex + 1) % resTotalSlides;
                updateResearchSlider(nextIndex);
            }, RES_AUTOPLAY_DELAY);
        };
        
        const stopResAutoplay = () => {
            if (resAutoplayTimer) {
                clearInterval(resAutoplayTimer);
                resAutoplayTimer = null;
            }
        };
        ...
        // Pause on hover
        resTrack.addEventListener('mouseenter', stopResAutoplay);
        resTrack.addEventListener('mouseleave', startResAutoplay);
        
        // Click dots
        resDotsContainer.addEventListener('click', (e) => {
            const targetDot = e.target.closest('.research-dot');
            if (!targetDot) return;
